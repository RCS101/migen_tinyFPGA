...
####
# Copied example written by whitequark
# Simulated by running python3 UART.py sim
# Loaded onto ice40_hx8k_b_evn board by running python3.py UART.py loopback
####
...
class UART(Module):
    def __init__(self, serial, clk_freq, baud_rate): # serial is -TestPads which has .rx and .tx dut = UART(pads, clk_freq=4800, baud_rate=1200)
        self.rx_data = Signal(8) # data to recieve register
        self.rx_ready = Signal()
        self.rx_ack = Signal()
        self.rx_error = Signal()

        self.tx_data = Signal(8) # data to send register
        ...
        self.comb += rx_strobe.eq(rx_counter == 0) # This results is rx_strobe high while rx_counter == 0. Else rx_strobe is low for all other values.
        self.sync += \
            If(rx_counter == 0,
                rx_counter.eq(divisor - 1) # reset counter - this divided the system clock to the correct baud.
            ).Else(
                rx_counter.eq(rx_counter - 1) # decrement the counter
            )

        self.rx_bitno = rx_bitno = Signal(3)
        self.submodules.rx_fsm = FSM(reset_state="IDLE")
        self.rx_fsm.act("IDLE",
            If(~serial.rx, # not serial.rx. is rx is low
                NextValue(rx_counter, divisor // 2),
                NextState("START") # change state to START
            )
        )
        self.rx_fsm.act("START",
            If(rx_strobe,           # rx_strobe is used as a rx clock
                NextState("DATA")
            )
        )
...
## I added comments cos I'm a noob to python and migen! They could be wrong!
def _test_rx(rx, dut):
    def T():
        yield; yield; yield; yield # let 4 clocks
    def B(bit):
        yield rx.eq(bit) # set rx bit
        yield from T()
    def S():
        yield from B(0)
        assert (yield dut.rx_error) == 0 # error checking
    ...
    def O(bits):
        yield from S() # I think this is just error checking, checking that rx_error and rx_ready both equal 0
        for bit in bits:
            yield from D(bit) # sets rx equal to bit with error checking.
        yield from E()

    def A(octet):
        yield from T()                          # 4 clocks
        assert (yield dut.rx_data) == octet     # set rx_data reg
        yield dut.rx_ack.eq(1)                  # set ack to 1
        while (yield dut.rx_ready) == 1: yield  # while rx_ready is 1 just clock (wait 1 clock)
        yield dut.rx_ack.eq(0)                  # reset ack
    def F():
        yield from T()
